...
data is a `list` containing the original graph, the compact graph and a map of
edge IDs that connect the two graphs to each other. A preprocessed sample graph
can be acessed with `road_data_sample`. After the preprocessing and routing is
done, `plot_map ()` can be used to display the traversal probabilities and
shortest path in a `shiny`/`leaflet` web app.
