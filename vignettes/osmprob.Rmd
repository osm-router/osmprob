...
downloading and preprocessing OpeneStreetMap[^1] road data on which the routing
...
```{r eval=FALSE}
    ...
    for vertex in all_vertices
        ...
        comps.insert (comp_num.at (vertex)) # insert current component number

        # insert all neighbouring component numbers of current vertex
        ...
        # invalid component number, so this neighbourhood is not in the list yet
        if largest == -1
            largest++
        ...
        # the largest component number
        for c in comp_num
            if (comps.contains (c) && c.component_number != -1
                comp_num.at (c) = largest
