...
18 different randomly selected test regions. To reduce the chance of faulty
outliers, the results of all speed measurements given are the median time of
runs.
...
the way between the same two points. As discussed above, high $\eta$ mean a low
...
map this almost looks like the traversal probabilities resembling the shortest
path. Only very close alternative routes that don't necessitate large detours
also have elevated probabilities. On the other hand, low $\eta$ lead to the
probabilities "seeping away" from the ideal path. This can be observed by
looking at the edges that are quite thin for high $\eta$, but constantly become
thicker with smaller $\eta$, which is consistent with the initial expectations
This effect also implies that the edges with higher probabilities for large
$\eta$ will decrease in probability as the others around it will increase.
...
graph does not restrict the number of times a given edge can be traversed.
...
path solutions at the example of Dijkstra's algorithm. The graphs also indicate
that different values for $\eta$ may also have an influence on calculation time.
Specifically, low $\eta$ and thus high global entropies appear to be leading to
longer calculation times. There are two negative outliers in the probabilistic
routing calculation times. As they both occurred on graphs with very few
vertices, these values show how big the influence of graph complexity on routing
calculation time is.
...
plt <- ggplot2::ggplot (data = dat) +
    ggplot2::geom_ribbon (ggplot2::aes (x = d_shortest, ymin = rbn_vals_mn,
                                        ymax = rbn_vals_mx),
                          fill = "#81A88D", alpha = 0.3) +
    ggplot2::geom_line (ggplot2::aes (x = d_shortest, y = t_prob,
                                      colour = factor (eta))) +
    ggplot2::geom_line (ggplot2::aes (x = d_shortest, y = t_shortest,
...
expected distances for different $\eta$, which are very high in comparison with
the standard deviation of the shortest paths (`r sd_short`). And like the
distances, their standard deviations are very sensitive to $\eta$. Another
possible reason for the spikes in the probabilistic distances could be the
number of vertices in the road graph, because a more complex road graph would
allow more movement between a start and end point than a very simple one would.
So on a very simple graph, all movement would be restricted on few edges, even
with high $\eta$.
...
```{r echo=FALSE, fig.cap="Figure 12: Comparison of probabilistic and shortest path solutions: graph complexity"}
ttl <- "Distance differences between shortest and probabilistic routes"
legendlab <- substitute (paste ("Global entropy [", eta, "]"))

plt <- ggplot2::ggplot (data = dat) +
    ggplot2::geom_line (ggplot2::aes (x = n_vtx_comp, y = d_prob,
                                      colour = factor (eta)), size = 1) +
    ggplot2::geom_point (ggplot2::aes (x = n_vtx_comp, y = d_prob),
                         size = 0.7) +
    ggplot2::geom_line (ggplot2::aes (x = n_vtx_comp, y = d_shortest,
                                      colour = "Shortest Path"), size = 1.5) +
    ggplot2::geom_point (ggplot2::aes (x = n_vtx_comp, y = d_shortest),
                        size = 0.7) +
    ggplot2::labs (title = ttl,
...
and not towards them. Another issue is the size of $\eta$ in the routing
process. Because there is no way yet to scale $\eta$ properly, the routing
results may be unrealistic in some cases and have to be used with caution.

# Discussion and outlook

The results show that `osmprob` can be used to adequately perform probabilistic
routing tasks. The resulting traversal probabilities appear to represent
realistic movement patterns for their respective $\eta$. A reliable way to
dimension $\eta$ for satisfactory results, however, is yet to be found. A
possible solution for this issue could be to compare real world movement data
with calculated results for different $\eta$ and investigate, which $\eta$ leads
to results best representing the data.
