...
plt <- ggplot2::ggplot (data = dat) +
    ...
    ggplot2::scale_x_continuous (trans = "log",
                                 breaks = c (0.001, 0.01, 0.1, 1, 7)) +
    ggplot2::scale_y_continuous (trans = "log",
                                 breaks = c (0.1, 1, 10, 100, 400))
...
calculation time is. The speed tests were run using a Intel i5-2400 CPU 3.10GHz
× 4 processor.
...
routing tasks and is therefore fit for use in real world applications. The
resulting traversal probabilities appear to represent realistic movement
patterns for their respective $\eta$. A reliable way to dimension $\eta$ for
satisfactory results, however, is yet to be found. A possible solution for this
issue could be to compare real world movement data with calculated results for
different $\eta$ and investigate, which $\eta$ leads to results best
